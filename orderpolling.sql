...
CREATE TABLE `notifyRecord` (
  ...
  `notifyTimes` int(11) NOT NULL DEFAULT 0 COMMENT '当前轮询通知次数',
  `notifyLimitTimes` int(11) NOT NULL DEFAULT 0 COMMENT '轮询通知总数',
  `url` varchar(2000) NOT NULL DEFAULT '' COMMENT '通知地址 获取订单处理状态',
  `orderTransactionNo` varchar(26) NOT NULL DEFAULT '' COMMENT '平台订单交易流水号',
  `status` varchar(10) NOT NULL COMMENT 'SUCCESS:成功 FAILED:失败',
  `notifyRule` varchar(100) DEFAULT NULL DEFAULT '' COMMENT '通知规则json',
  PRIMARY KEY (`id`),
...
CREATE TABLE `order` (
  ...
  `orderTransactionNo` varchar(26) NOT NULL DEFAULT '' COMMENT '平台订单交易流水号',
  `status` varchar(15) NOT NULL COMMENT 'SUCCESS:成功 FAILED:失败 WAITING_PAYMENT:等待支付',
  PRIMARY KEY (`id`),
  UNIQUE KEY `u1` (`orderTransactionNo`),
  KEY `k1` (`createTime`)
